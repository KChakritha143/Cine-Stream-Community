...
app.get("/", async (req, res) => {
  try {
    ...
    console.error("Error querying total posts:", error);
    res.status(500).json({
      success: false,
...
app.get(["/posts/:id", "/api/posts/:id"], async (req, res) => {
  try {
    const { id } = req.params;
...
app.put(["/posts/:id", "/api/posts/:id"], async (req, res) => {
  try {
    const { id } = req.params;
...
app.delete(["/posts/:id", "/api/posts/:id"], async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(404).json({
