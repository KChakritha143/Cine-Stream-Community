...
async function runTests() {
  ...
  try {
    ...
    assert.strictEqual(postResult.post.publisher, "Test Publisher");
    const postId = postResult.post.id;
    console.log(`POST /posts passed. Created Post ID: ${postId}\n`);

    console.log("Testing POST /posts (Create post with default publisher)...");
    const defaultPostPayload = {
      title: "Second Post",
      content: "Content for the second post without publisher.",
    };
    const defaultPostRes = await fetch(`${BASE_URL}/posts`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(defaultPostPayload),
    });
    assert.strictEqual(defaultPostRes.status, 201);
    const defaultPostResult = await defaultPostRes.json();
    assert.strictEqual(defaultPostResult.post.publisher, "Anonymous Fan");
    const defaultPostId = defaultPostResult.post.id;
    console.log(`POST /posts with default publisher passed. Created Post ID: ${defaultPostId}\n`);

    console.log("Testing GET /posts (Verify posts exist)...");
    ...
    assert.strictEqual(postsAfterPost.length, 2);
    assert.ok(postsAfterPost.some(p => p.id === postId));
    assert.ok(postsAfterPost.some(p => p.id === defaultPostId));
    ...
    assert.strictEqual(singlePost.publisher, "Test Publisher");
    ...
    console.log(`Testing DELETE /posts/${postId} (Delete first post)...`);
    const deleteRes = await fetch(`${BASE_URL}/posts/${postId}`, {
      method: "DELETE",
    });
    assert.strictEqual(deleteRes.status, 200);
    console.log("DELETE /posts/:id (first post) passed.\n");

    console.log(`Testing DELETE /posts/${defaultPostId} (Delete second post)...`);
    const deleteDefaultRes = await fetch(`${BASE_URL}/posts/${defaultPostId}`, {
      method: "DELETE",
    });
    assert.strictEqual(deleteDefaultRes.status, 200);
    console.log("DELETE /posts/:id (second post) passed.\n");
